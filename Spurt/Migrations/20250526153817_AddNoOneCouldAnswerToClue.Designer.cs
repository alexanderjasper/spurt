// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Spurt.Data;

#nullable disable

namespace Spurt.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20250526153817_AddNoOneCouldAnswerToClue")]
    partial class AddNoOneCouldAnswerToClue
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "9.0.0-preview.3.24172.4");

            modelBuilder.Entity("Spurt.Domain.Categories.Category", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<bool>("IsSubmitted")
                        .HasColumnType("INTEGER");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("TEXT");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("PlayerId")
                        .IsUnique();

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("Spurt.Domain.Categories.Clue", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<string>("Answer")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<Guid?>("AnsweredByPlayerId")
                        .HasColumnType("TEXT");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("TEXT");

                    b.Property<bool>("NoOneCouldAnswer")
                        .HasColumnType("INTEGER");

                    b.Property<int>("PointValue")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Question")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("AnsweredByPlayerId");

                    b.HasIndex("CategoryId");

                    b.ToTable("Clues");
                });

            modelBuilder.Entity("Spurt.Domain.Games.Game", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<Guid?>("BuzzedPlayerId")
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("BuzzedTime")
                        .HasColumnType("TEXT");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<Guid?>("CurrentChoosingPlayerId")
                        .HasColumnType("TEXT");

                    b.Property<Guid?>("SelectedClueId")
                        .HasColumnType("TEXT");

                    b.Property<int>("State")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("BuzzedPlayerId");

                    b.HasIndex("SelectedClueId");

                    b.ToTable("Games");
                });

            modelBuilder.Entity("Spurt.Domain.Players.Player", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<Guid>("GameId")
                        .HasColumnType("TEXT");

                    b.Property<bool>("IsCreator")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(false);

                    b.Property<Guid>("UserId")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("GameId");

                    b.HasIndex("UserId");

                    b.ToTable("Players");
                });

            modelBuilder.Entity("Spurt.Domain.Users.User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("Spurt.Domain.Categories.Category", b =>
                {
                    b.HasOne("Spurt.Domain.Players.Player", "Player")
                        .WithOne("Category")
                        .HasForeignKey("Spurt.Domain.Categories.Category", "PlayerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Player");
                });

            modelBuilder.Entity("Spurt.Domain.Categories.Clue", b =>
                {
                    b.HasOne("Spurt.Domain.Players.Player", "AnsweredByPlayer")
                        .WithMany("AnsweredClues")
                        .HasForeignKey("AnsweredByPlayerId");

                    b.HasOne("Spurt.Domain.Categories.Category", "Category")
                        .WithMany("Clues")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AnsweredByPlayer");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("Spurt.Domain.Games.Game", b =>
                {
                    b.HasOne("Spurt.Domain.Players.Player", "BuzzedPlayer")
                        .WithMany()
                        .HasForeignKey("BuzzedPlayerId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Spurt.Domain.Categories.Clue", "SelectedClue")
                        .WithMany()
                        .HasForeignKey("SelectedClueId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("BuzzedPlayer");

                    b.Navigation("SelectedClue");
                });

            modelBuilder.Entity("Spurt.Domain.Players.Player", b =>
                {
                    b.HasOne("Spurt.Domain.Games.Game", "Game")
                        .WithMany("Players")
                        .HasForeignKey("GameId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Spurt.Domain.Users.User", "User")
                        .WithMany("Players")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Game");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Spurt.Domain.Categories.Category", b =>
                {
                    b.Navigation("Clues");
                });

            modelBuilder.Entity("Spurt.Domain.Games.Game", b =>
                {
                    b.Navigation("Players");
                });

            modelBuilder.Entity("Spurt.Domain.Players.Player", b =>
                {
                    b.Navigation("AnsweredClues");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("Spurt.Domain.Users.User", b =>
                {
                    b.Navigation("Players");
                });
#pragma warning restore 612, 618
        }
    }
}
